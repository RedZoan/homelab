<#
.SYNOPSIS
    Compares media files between two directories and reports files that exist only in the source.

.DESCRIPTION
    Scans a source and destination directory for media files (.ts, .mp4, .mkv), then identifies
    files in the source that have NO matching counterpart in the destination based on a "cleaned"
...
.PARAMETER OutputFile
    The full path for the output CSV file (e.g., C:\Reports\NonMatchedFiles.csv).
...
$OutputFile = Read-Host "Enter the FULL path for the OUTPUT CSV file (e.g., C:\Reports\NonMatchedFiles.csv)"
...
# List to store the non-matched files
$NonMatchedFiles = @()
...
foreach ($sourceFile in $SourceFiles) {
    ...
    # Check if the cleaned source filename DOES NOT exist in our destination lookup table
    if (-not $DestinationFileLookup.ContainsKey($cleanedSourceName)) {
        # If no match is found, add the source file's information to our list
        $NonMatchedFiles += [PSCustomObject]@{
            ...
            # No DestinationFilePath/Extension as there's no match
        }
    }
}

# Display the non-matched files on the screen
if ($NonMatchedFiles.Count -gt 0) {
    Write-Host "`n--- Non-Matched Files in Source ---"
    # Display relevant columns for clarity
    $NonMatchedFiles | Format-Table -Property SourceFileName, SourceFileExtension, CleanedFileName -AutoSize
    Write-Host "-----------------------------------`n"

    # Export the non-matched files to a CSV
    $NonMatchedFiles | Export-Csv -Path $OutputFile -NoTypeInformation -Encoding UTF8
    Write-Host "Comparison complete. Found $($NonMatchedFiles.Count) non-matching files."
    Write-Host "Results exported to: $OutputFile"
} else {
    Write-Host "All source files have a match in the destination directory based on the comparison criteria."
}
